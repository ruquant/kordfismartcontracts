# Based on https://gitlab.com/SmartPy/smartpy/-/blob/master/python/templates/FA1.2.py
# Fixed transfer entrypoint, errors, storage according specification. 
...
class FA12_core(sp.Contract, FA12_common):
    def __init__(self, config, **extra_storage):
        ...
        self.init(
            ledger = sp.big_map(tvalue = sp.TRecord(approvals = sp.TMap(sp.TAddress, sp.TNat), balance = sp.TNat)),
            totalSupply = 0,
            **extra_storage
        )
    ...
    def transfer(self, params):
        ...
        with sp.if_(params.from_ != sp.sender):
            sp.verify(
                self.data.ledger[params.from_].approvals.contains(sp.sender) 
                    & (self.data.ledger[params.from_].approvals[sp.sender] >= params.value),
                FA12_Error.NotEnoughAllowance
            )
        sp.verify(self.data.ledger[params.from_].balance >= params.value, FA12_Error.NotEnoughBalance)
...
class FA12_mint_burn(FA12_core):
    ...
    def burn(self, params):
        sp.set_type(params, sp.TRecord(address = sp.TAddress, value = sp.TNat))
        sp.verify(self.is_administrator(sp.sender), FA12_Error.NotAdmin)
        sp.verify(self.data.ledger[params.address].balance >= params.value, FA12_Error.NotEnoughBalance)
...
class FA12_token_metadata(FA12_core):
    """
        ...
        >>    "decimals"  : Should be an integer (converted to a UTF-8 string in decimal) which defines the position of                   the decimal point in token ledger for displaypurposes.
    """
