...
class FlashLoanTest(MainContractBaseTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.main_contract.context.key = Key.from_encoded_key(BOB_KEY)
        cls.main_contract.depositLending().with_amount(100_000_000).send(gas_reserve=10000, min_confirmations=1)
        ...
        cls.flash_loaner.context.key = Key.from_encoded_key(ALICE_KEY)
        cls.flash_loaner.default().with_amount(1_000_000).send(gas_reserve=10000, min_confirmations=1)

    def test_basic(self):
        with self.assertRaises(MichelsonError) as context:
            self.flash_loaner.context.key = Key.from_encoded_key(BOB_KEY)
            self.flash_loaner.try_flashloan_xtz(100, 0).send(gas_reserve=10000, min_confirmations=1)
        ...
        with self.assertRaises(MichelsonError) as context:
            self.flash_loaner.context.key = Key.from_encoded_key(BOB_KEY)
            self.flash_loaner.try_flashloan_xtz(100_578_999, 100_000_000).send(gas_reserve=10000, min_confirmations=1)
        self.assertEqual(str(context.exception.args[0]['with']['string']), 'loan error')

        self.flash_loaner.context.key = Key.from_encoded_key(BOB_KEY)
        self.flash_loaner.try_flashloan_xtz(100_579_000, 100_000_000).send(gas_reserve=10000, min_confirmations=1)

        self.assertEqual(self.flash_loaner.context.get_balance(), 421_000)
        self.assertEqual(self.flash_loaner.storage['balance'](), 101_000_000)
        self.assertEqual(self.main_contract.context.get_balance(), 100_579_000)
