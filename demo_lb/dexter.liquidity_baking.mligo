    ...
    if Tezos.now >= deadline then
      (failwith error_THE_CURRENT_TIME_MUST_BE_LESS_THAN_THE_DEADLINE : result)    
        ...
        // Proceed to form the operations and update the storage
            ...
            let new_xtzPool = match storage.xtzPool - xtz_withdrawn with
                | None -> (failwith 10999n)
                | Some n -> n in
   ...
   let { to_ = to_ ;
	...
	let tokens_bought =
            ...
            if bought < minTokensBought then
                ...
                bought)
        in
        let new_tokenPool = (match is_a_nat (storage.tokenPool - tokens_bought) with
...
let token_to_xtz (param : token_to_xtz) (storage : storage) =
    ...
    if Tezos.now >= deadline then
        ...
        let new_xtzPool = match storage.xtzPool - xtz_bought with
            | None -> (failwith 10999n)
            | Some n -> n in
